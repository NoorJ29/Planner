...
with sync_playwright() as p:
    ...
    card=".side-foot .pcard"
    check("6. laptop profile card",pg.inner_text(card+" > span")=="Noor" and pg.inner_text(card+" .pmail")=="me@example.com" and pg.inner_text(card+" .pav em")=="N",pg.inner_text(card))
    dot=lambda sel:pg.evaluate("s=>getComputedStyle(document.querySelector(s+' .pav > i')).backgroundColor",sel)
    green,red=dot(card),None
    check("6. green dot when synced",pg.get_attribute(card,"data-dot")=="ok" and green=="rgb(30, 138, 76)",green)
    pg.hover(card); pg.wait_for_timeout(250)
    check("6. hover shows Synced",pg.inner_text(card+" .tip")=="Synced" and pg.evaluate("getComputedStyle(document.querySelector('.pcard .tip')).opacity")=="1")
    pg.screenshot(path="tests/out/profile_desk.png")
    ctx.set_offline(True); pg.wait_for_timeout(300); red=dot(card)
    check("6. red dot when offline",pg.get_attribute(card,"data-dot")=="bad" and red=="rgb(184, 67, 58)" and pg.inner_text(card+" .tip").startswith("Not synced: you're offline"),red)
    pg.screenshot(path="tests/out/profile_desk_offline.png")
    ctx.set_offline(False); pg.wait_for_timeout(300)
    check("6. green again when back online",pg.get_attribute(card,"data-dot")=="ok")
    pg.mouse.move(700,450); pg.click("#sideSettings"); pg.wait_for_timeout(200)
    check("6. sidebar Settings button",pg.inner_text("#bigDate")=="Settings" and pg.get_attribute("#sideSettings","aria-current")=="page" and pg.locator('#sideNav button[aria-current="page"]').count()==0)
    pg.click("#sideGuide"); pg.wait_for_timeout(200)
    check("6. ? button opens the guide",pg.inner_text("#bigDate")=="Guide")
    pg.click(card); pg.wait_for_timeout(200)
    check("6. profile card opens profile",pg.inner_text("#bigDate")=="Profile")
    pg.set_viewport_size({"width":390,"height":844}); pg.wait_for_timeout(300)
    check("6. phone avatar dot green",pg.get_attribute("#sync","data-dot")=="ok" and dot("#sync")=="rgb(30, 138, 76)")
    pg.click("#sync",button="right"); check("6. long-press shows status",toast(pg)=="Synced")
    ...
    pg.keyboard.press("Escape"); pg.set_viewport_size({"width":1400,"height":900}); pg.wait_for_timeout(300)
    check("11. signed-out card",pg.inner_text(".side-foot .pcard > span")=="Sign in" and pg.inner_text(".side-foot .pcard .pmail")=="to sync your devices" and pg.locator(".side-foot .pcard .pav.anon svg").count()==1,pg.inner_text(".side-foot .pcard"))
    pg.screenshot(path="tests/out/profile_desk_signedout.png")
    b.close()
print("ERRORS:",errs)
